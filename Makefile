...
build:
	...
	$(GO) build -v -o bin/api $(LDFLAGS)

gen-mocks:
	mockery -dir=./bindings/ -all
	mockery -dir=./endpoints/ -all
	mockery -dir=./database/ -all
...
test:
	...
	$(GO) test -ldflags -s -v ./database -race -cover -coverprofile=$(COVERAGEDIR)/database.coverprofile
...
cover:
	...
	go tool cover -html=$(COVERAGEDIR)/database.coverprofile -o $(COVERAGEDIR)/database.html
